...
- The theme switch sits at the top right of the brand header; search uses the full width below. Brand and banners scroll away. IntersectionObserver changes the single category navigation into a fixed compact strip; a measured placeholder preserves its document space. Search remains sticky above it.
...
- Cards use a landscape image above the existing dollar prices, actual stock and optional plan/duration, with Details and Buy actions. WhatsApp checkout uses the existing number and encoded message. Stock at or below zero disables Buy.
